    ...
    public class YM2151
    {
        ...
        */
        ...
        public static void irqAon_callback()
        {
            ...
            if (oldstate == 0)
            {
                //Cpuint.cpunum_set_input_line_and_vector1001();                
                PSG.irqhandler(1);
            }
        }
        ...
        public static void irqAoff_callback()
        {
            ...
            if (oldstate == 1)
            {
                //Cpuint.cpunum_set_input_line_and_vector1000();
                PSG.irqhandler(0);
            }
        }
                    ...
                    switch (r)
                    {
                        ...
                        case 0x14:	/* CSM, irq flag reset, irq enable, timer start/stop */
                            ...
                            if ((v & 0x10) != 0)	/* reset timer A irq flag */
                            {
                                ...
                                /*Atime time1 = Cpuexec.cpunum_get_localtime1();
                                Timer.emu_timer timer=new Timer.emu_timer();

                                timer.callback = irqAoff_callback;
                                timer.temporary = true;
                                timer.func = "irqAoff_callback";
                                timer.start = time1;

                                if (timer == Timer.callback_timer)
                                {
                                    Timer.callback_timer_modified = true;
                                }
                                timer.enabled = true;
                                timer.start = time1;
                                timer.expire = time1;
                                timer.period =Attotime.attotime_never;  

                                Timer.timer_list_insert(timer);
                                if (Timer.lt.IndexOf(timer) == 0)
                                {
                                    Cpuexec.activecpu_abort_timeslice1();
                                }*/
                            }
                            ...
                            {	/* load and start timer A */
                                ...
                                if (!Timer.timer_enable(PSG.timer_A, true))
                                {
                                    Timer.timer_adjust_periodic(PSG.timer_A, PSG.timer_A_time[PSG.timer_A_index], Attotime.ATTOTIME_NEVER);
                                    /*Atime time1 = Cpuexec.cpunum_get_localtime1();
                                    if (PSG.timer_A == Timer.callback_timer)
                                    {
                                        Timer.callback_timer_modified = true;
                                    }
                                    PSG.timer_A.enabled = true;
                                    PSG.timer_A.start = time1;
                                    PSG.timer_A.expire = Attotime.attotime_add(time1, PSG.timer_A_time[PSG.timer_A_index]);
                                    PSG.timer_A.period = Attotime.attotime_never;
                                    if (Timer.lt.IndexOf(PSG.timer_A) == 0)
                                    {
                                        Cpuexec.activecpu_abort_timeslice1();
                                    }*/
                                    PSG.timer_A_index_old = PSG.timer_A_index;
