        ...
        private void CHECK_IRQ_LINES()
        {
            ...
            else if (irq_state[M6809_IRQ_LINE] != LineState.CLEAR_LINE && (CC & CC_II) == 0)
            {
                ...
                else
                {
                    ...
                    PUSHBYTE(CC);
                    extra_cycles += 19;
                    /*StreamWriter sw2 = new StreamWriter(@"\VS2008\compare1\compare1\bin\Debug\ex2.txt", true);
                    sw2.WriteLine(Cpuexec.activecpu.ToString("x2") + "\t" + Timer.global_basetime.seconds.ToString("x") + "\t" + Timer.global_basetime.attoseconds.ToString("x"));
                    sw2.Close();*/
                }
                CC |= CC_II;
