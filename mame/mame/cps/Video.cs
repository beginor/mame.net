...
namespace mame
{
    public partial class CPS
    {
        ...
        private static void cps1_get_video_base()
        {
            ...
            Tmap.ttmap[0].rowscroll[0] = cps_a_regs[CPS1_SCROLL1_SCROLLX] + scroll1xoff;
            Tmap.ttmap[0].colscroll[0] = cps_a_regs[CPS1_SCROLL1_SCROLLX];
            Tmap.ttmap[1].rowscroll[0] = cps_a_regs[CPS1_SCROLL2_SCROLLX] + scroll1xoff;
            Tmap.ttmap[1].colscroll[0] = cps_a_regs[CPS1_SCROLL2_SCROLLX];
            Tmap.ttmap[2].rowscroll[0] = cps_a_regs[CPS1_SCROLL3_SCROLLX] + scroll1xoff;
            Tmap.ttmap[2].colscroll[0] = cps_a_regs[CPS1_SCROLL3_SCROLLX];
            ...
            Tmap.ttmap[0].enable = ((layercontrol & layer_enable_mask[0]) != 0);
            Tmap.ttmap[1].enable = ((layercontrol & layer_enable_mask[1]) != 0 && (videocontrol & 4) != 0);
            Tmap.ttmap[2].enable = ((layercontrol & layer_enable_mask[2]) != 0 && (videocontrol & 8) != 0);
        ...
        private static void cps1_gfxram_w(int offset)
        {
            ...
            if (page == (cps_a_regs[CPS1_SCROLL1_BASE] & 0x3c0))
            {
                ...
                row += memindex % 0x20;
                col = memindex / 0x20;
                Tmap.ttmap[0].tilemap_mark_tile_dirty(row, col);
            }
            if (page == (cps_a_regs[CPS1_SCROLL2_BASE] & 0x3c0))
            {
                ...
                row += memindex % 0x10;
                col = memindex / 0x10;
                Tmap.ttmap[1].tilemap_mark_tile_dirty(row, col);
            }
            if (page == (cps_a_regs[CPS1_SCROLL3_BASE] & 0x3c0))
            {
                ...
                row += memindex % 0x08;
                col = memindex / 0x08;
                Tmap.ttmap[2].tilemap_mark_tile_dirty(row, col);
            }
        }
        private static void cps1_update_transmasks()
        {
            ...
            for (group = 0; group < 4; group++)
            {
                ...
                for (pen = 0; pen < 16; pen++)
                {
                    ...
                    if (Tmap.ttmap[0].pen_to_flags[group, pen] != layermask)
                    {
                        Tmap.ttmap[0].pen_to_flags[group, pen] = layermask;
                        Tmap.ttmap[0].all_tiles_dirty = true;
                    }
                    if (Tmap.ttmap[1].pen_to_flags[group, pen] != layermask)
                    {
                        Tmap.ttmap[1].pen_to_flags[group, pen] = layermask;
                        Tmap.ttmap[1].all_tiles_dirty = true;
                    }
                    if (Tmap.ttmap[2].pen_to_flags[group, pen] != layermask)
                    {
                        Tmap.ttmap[2].pen_to_flags[group, pen] = layermask;
                        Tmap.ttmap[2].all_tiles_dirty = true;
                    }
                }
            }
                ...
                for (offs = 0; offs < 0x2000 / 2; offs++)
                {
                    ...
                    if (col != 0x0f)
                    {
                        ...
                        if (sx >= Video.screenstate.visarea.min_x && sx <= Video.screenstate.visarea.max_x && sy >= Video.screenstate.visarea.min_y && sy <= Video.screenstate.visarea.max_y)
                            Video.bitmapbaseC[Video.curbitmap][sy * 0x200 + sx] = (ushort)(0xa00 + col);
                    }
                }
            }
            if (cps1_stars_enabled[1] != 0)
            {
                for (offs = 0; offs < 0x2000 / 2; offs++)
                {
                    ...
                    if (col != 0x0f)
                    {
                        ...
                        col = (int)(((col & 0xe0) >> 1) + (Video.screenstate.frame_number / 16 & 0x0f));
                        if (sx >= Video.screenstate.visarea.min_x && sx <= Video.screenstate.visarea.max_x && sy >= Video.screenstate.visarea.min_y && sy <= Video.screenstate.visarea.max_y)
                            Video.bitmapbaseC[Video.curbitmap][sy * 0x200 + sx] = (ushort)(0x800 + col);
                    }
                }
            }
        }
        private static void cps1_render_layer(int layer)
        {
            switch (layer)
            {
                case 0:
                    ...
                    break;
                case 1:
                    Tmap.ttmap[0].tilemap_draw_primask(Tilemap.videovisarea, 0);
                    break;
                case 2:
                    Tmap.ttmap[1].tilemap_draw_primask(Tilemap.videovisarea, 0);
                    break;
                case 3:
                    Tmap.ttmap[2].tilemap_draw_primask(Tilemap.videovisarea, 0);
                    break;
            }
        }
        private static void cps1_render_layer(int layer, byte primask)
        {
            switch (layer)
            {
                case 0:
                    ...
                    break;
                case 1:
                    Tmap.ttmap[0].tilemap_draw_primask(Tilemap.videovisarea, primask);
                    break;
                case 2:
                    Tmap.ttmap[1].tilemap_draw_primask(Tilemap.videovisarea, primask);
                    break;
                case 3:
                    Tmap.ttmap[2].tilemap_draw_primask(Tilemap.videovisarea, primask);
                    break;
            }
        }
        private static void cps1_render_high_layer(int layer)
        {
            switch (layer)
            {
                case 0:
                    break;
                case 1:
                    Tmap.ttmap[0].tilemap_draw_primask4(Tilemap.videovisarea);
                    break;
                case 2:
                    Tmap.ttmap[1].tilemap_draw_primask4(Tilemap.videovisarea);
                    break;
                case 3:
                    Tmap.ttmap[2].tilemap_draw_primask4(Tilemap.videovisarea);
                    break;
            }
        }
        public static void video_update_cps1()
        {
            ...
            cps1_update_transmasks();
            Tmap.ttmap[0].tilemap_set_scrollx(0, scroll1x);
            Tmap.ttmap[0].tilemap_set_scrolly(0, scroll1y);
            if ((videocontrol & 0x01)!=0)	/* linescroll enable */
            {
                int scrly = -scroll2y;
                int otheroffs;
                Tmap.ttmap[1].scrollrows = 1024;
                otheroffs = cps_a_regs[CPS1_ROWSCROLL_OFFS];
                for (i = 0; i < 0x400; i++)
                {
                    Tmap.ttmap[1].tilemap_set_scrollx((i - scrly) & 0x3ff, scroll2x + gfxram[(other + ((otheroffs + i) & 0x3ff)) * 2] * 0x100 + gfxram[(other + ((otheroffs + i) & 0x3ff)) * 2 + 1]);
                }
            }
            else
            {
                Tmap.ttmap[1].scrollrows = 1;               
                Tmap.ttmap[1].tilemap_set_scrollx(0, scroll2x);
            }
            Tmap.ttmap[1].tilemap_set_scrolly(0, scroll2y);
            Tmap.ttmap[2].tilemap_set_scrollx(0, scroll3x);
            Tmap.ttmap[2].tilemap_set_scrolly(0, scroll3y);
            Array.Copy(uuBFF, Video.bitmapbaseC[Video.curbitmap], 0x40000);
            cps1_render_stars();
