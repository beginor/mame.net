    ...
    public partial class Tmap
    {
        ...
        public void tile_updateC0(int col, int row)
        {
            ...
            memindex = (row & 0x1f) + ((col & 0x3f) << 5) + ((row & 0x20) << 6);
            {
                ...
                code %= Tmap.ttmap[0].total_elements;
        ...
        private void tile_updateC1(int col, int row)
        {
            byte group1, flags1;
            int x0 = 0x10 * col;
            int y0 = 0x10 * row;
            ...
            memindex = (row & 0x0f) + ((col & 0x3f) << 4) + ((row & 0x30) << 6);
            {
                ...
                code %= Tmap.ttmap[1].total_elements;
        ...
        private void tile_updateC2(int col, int row)
        {
            byte group2, flags2;
            int x0 = 0x20 * col;
            int y0 = 0x20 * row;
            ...
            memindex = (row & 0x07) + ((col & 0x3f) << 3) + ((row & 0x38) << 6);
            {
                ...
                code %= Tmap.ttmap[2].total_elements;
            ...
            for (; ; )
            {
                ...
                for (column = mincol; column <= maxcol; column++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            cur_trans = ((flagsmap[offsety1, column * tilewidth] & 0x2f) == 0x20) ? trans_t.WHOLLY_OPAQUE : trans_t.WHOLLY_TRANSPARENT;
                        }
                    }
                    ...
                    if (prev_trans != trans_t.WHOLLY_TRANSPARENT)
                    {
                        ...
                        if (prev_trans == trans_t.WHOLLY_OPAQUE)
                        {
                            ...
                            {
                                Array.Copy(pixmap, offsety2 * width + x_start, Video.bitmapbaseC[Video.curbitmap], (offsety2 + ypos) * 0x200 + xpos + x_start, x_end - x_start);
                                if (priority != 0)
                                {
                        ...
                        else if (prev_trans == trans_t.MASKED)
                        {
                            for (cury = y; cury < nexty; cury++)
                            {
                                for (i = xpos + x_start; i < xpos+x_end; i++)
                                {
                                    if ((flagsmap[offsety2, i - xpos] & 0x2f) == 0x20)
                                    {
                                        Video.bitmapbaseC[Video.curbitmap][(offsety2 + ypos) * 0x200 + i] = pixmap[offsety2 * width + i - xpos];
                                        Tilemap.priority_bitmap[offsety2 + ypos, i] = (byte)(Tilemap.priority_bitmap[offsety2 + ypos, i] | priority);
                                    }
                                }
                                offsety2++;
