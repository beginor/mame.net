...
namespace mame
{
    public partial class Tmap
    {
        ...
        public void tile_updatePgmtx(int col, int row)
        {
            ...
            byte flags;
            tile_index = row * pgm_tx_tilemap.cols + col;
            ...
            code = tileno % pgm_tx_tilemap.total_elements;
            flags = (byte)(flipyx & 3);
        ...
        public void tilemap_draw_instancePgmtx(RECT cliprect, int xpos, int ypos)
        {
            int mincol, maxcol;
            int x1, y1, x2, y2;
            int y, nexty;
                    ...
                    if (prev_trans != trans_t.WHOLLY_TRANSPARENT)
                    {
                        ...
                        if (prev_trans == trans_t.WHOLLY_OPAQUE)
                        {
                            for (cury = y; cury < nexty; cury++)
                            {
                                ...
                                {
                                    Video.bitmapbasePGM[Video.curbitmap][(offsety2 + ypos) * 0x200 + i] = (ushort)(pixmap[offsety2 * 0x200 + i - xpos] + palette_offset);
        ...
        public void tile_updatePgmbg(int col, int row)
        {
            ...
            code = tileno % pgm_bg_tilemap.total_elements;
            flags = (byte)(flipyx & 3);
            tileflags[row, col] = tile_drawPgmbg(code * 0x400, x0, y0, 0x400, flags);
        }
        public byte tile_drawPgmbg(int pendata_offset, int x0, int y0, int palette_base, byte flags)
        {
            ...
            for (ty = 0; ty < height; ty++)
            {
                ...
                for (tx = 0; tx < width; tx++)
                {
                    ...
                    map = pen_to_flags[0, pen];
                    offset1++;
                    pixmap[offsety1 * 0x800 + x0 + xoffs] = (ushort)(0x400 + pen);
                    flagsmap[offsety1, x0 + xoffs] = map;
                    andmask &= map;
                    ormask |= map;
                    xoffs += dx0;
        ...
        public void tilemap_draw_instancePgmbg(RECT cliprect, int xpos, int ypos)
        {
            int mincol, maxcol;
            int x1, y1, x2, y2;
            int y, nexty;
                    ...
                    if (prev_trans != trans_t.WHOLLY_TRANSPARENT)
                    {
                        ...
                        if (prev_trans == trans_t.WHOLLY_OPAQUE)
                        {
                            for (cury = y; cury < nexty; cury++)
                            {
                                for (i = xpos + x_start; i < xpos + x_end; i++)
                                {
                                    Video.bitmapbasePGM[Video.curbitmap][(offsety2 + ypos) * 0x200 + i] = (ushort)(pixmap[offsety2 * 0x800 + i - xpos] + palette_offset);
                                    Tilemap.priority_bitmap[offsety2 + ypos, i] = priority;
                                }
                                offsety2++;
                            }
