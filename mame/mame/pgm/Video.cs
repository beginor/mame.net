...
namespace mame
{
    public partial class PGM
    {
        ...
        private static void pgm_draw_pix(int xdrawpos, int ydrawpos, int pri, ushort srcdat)
        {
            if ((xdrawpos >= 0) && (xdrawpos < 448))
            {
                if ((Tilemap.priority_bitmap[ydrawpos, xdrawpos] & 1) == 0)
                {
                    if (pri == 0)
                    {
                        Video.bitmapbasePGM[Video.curbitmap][ydrawpos * 0x200 + xdrawpos] = srcdat;
                    }
                    else
                    {
        ...
        private static void draw_sprite_line(int wide, int ydrawpos, int xzoom, int xgrow, int flip, int xpos, int pri, int realxsize,int aoffset,int boffset, int palt, int draw)
        {
            int xcnt, xcntdraw;
            int xzoombit;
            int xoffset = 0;
            ...
            {
                int x;
                msk = ((sprmaskrom[(boffset + 1) & bdatasize] << 8) | (sprmaskrom[(boffset + 0) & bdatasize] << 0));
                for (x = 0; x < 16; x++)
                {
                    if ((msk & 0x0001)==0)
                    {
                        srcdat = (ushort)(pgm_sprite_a_region[aoffset & adatasize] + palt * 32);
                        aoffset++;
                        if (draw!=0)
                        {
                            ...
                            if (xzoombit == 1 && xgrow == 1)
                            {
                                ...
                                if (ydrawpos != -10000)
                                {
                                    pgm_draw_pix(xdrawpos, ydrawpos, pri, srcdat);
                                }
                                xcntdraw++;
                                if ((flip & 0x01)==0)
                                    xdrawpos = xpos + xcntdraw;
        ...
        private static void draw_sprite_new_zoomed(int wide, int high, int xpos, int ypos, int palt, int boffset, int flip, int xzoom, int xgrow, int yzoom, int ygrow, int pri)
        {
            int ycnt;
            int ydrawpos;
            int ycntdraw;
            ...
            int bdatasize = sprmaskrom.Length - 1;
            int aoffset;
            aoffset = (sprmaskrom[(boffset + 3) & bdatasize] << 24) | (sprmaskrom[(boffset + 2) & bdatasize] << 16) |
                        (sprmaskrom[(boffset + 1) & bdatasize] << 8) | (sprmaskrom[(boffset + 0) & bdatasize] << 0);
            aoffset = aoffset >> 2;
            aoffset *= 3;
            boffset += 4;
            ycnt = 0;
            ...
            while (ycnt < high)
            {
                ...
                if (yzoombit == 1 && ygrow == 1)
                {
                    int temp_aoffset = aoffset;
                    int temp_boffset = boffset;
                    if ((flip & 0x02) == 0)
                    ...
                    if ((ydrawpos >= 0) && (ydrawpos < 224))
                    {
                        ...
                        draw_sprite_line(wide, ydrawpos, xzoom, xgrow, flip, xpos, pri, realxsize, aoffset, boffset, palt, 1);
                    }
                    else
                    {
                    ...
                    ycntdraw++;
                    aoffset = temp_aoffset;
                    boffset = temp_boffset;
                    if ((flip & 0x02)==0)
                        ydrawpos = ypos + ycntdraw;
                    else
                        ydrawpos = ypos + realysize - ycntdraw;
        ...
        private static void draw_sprite_line_basic(int wide, int ydrawpos, int flip, int xpos, int pri, int realxsize, int aoffset, int boffset, int palt, int draw)
        {
            int xcnt, xcntdraw;
            int xoffset = 0;
            int xdrawpos = 0;
                    ...
                    for (x = 0; x < 16; x++)
                    {
                        if ((msk & 0x0001)==0)
                        {
                            ...
                            if (draw!=0)
                            {
                                ...
                                if (ydrawpos != -10000)
                                {
                                    pgm_draw_pix_nopri(xdrawpos, ydrawpos, srcdat);
                                }
                                xcntdraw++;
                            }
                        }
                    ...
                    for (x = 0; x < 16; x++)
                    {
                        if ((msk & 0x0001)==0)
                        {
                            ...
                            if (draw!=0)
                            {
                                ...
                                if (ydrawpos != -10000)
                                {
                                    pgm_draw_pix_pri(xdrawpos, ydrawpos, srcdat);
                                }
                                xcntdraw++;
                            }
                        }
        ...
        private static void draw_sprite_new_basic(int wide, int high, int xpos, int ypos, int palt, int boffset, int flip, int pri)
        {
            int ycnt;
            int ydrawpos;
            int ycntdraw;
            ...
            for(ycnt=0;ycnt<high;ycnt++)
            {
                ...
                if ((ydrawpos >= 0) && (ydrawpos < 224))
                {
                    draw_sprite_line_basic(wide, ydrawpos, flip, xpos, pri, realxsize,aoffset,boffset, palt, 1);
                }
                else
                {
                    //draw_sprite_line_basic(wide, -10000, flip, xpos, pri, realxsize, aoffset, boffset, palt, 0);
                    if ((flip & 0x02)==0)
                    {
                        if (ydrawpos > 224)
                            return;
        ...
        private static void draw_sprites1()
        {
            int i;
            for (i = 0; i < 0x500; i += 5)
            {
                int xpos = pgm_spritebufferram[i + 0] & 0x07ff;
                int ypos = pgm_spritebufferram[i + 1] & 0x03ff;
                int xzom = (pgm_spritebufferram[i + 0] & 0x7800) >> 11;
                int xgrow = (pgm_spritebufferram[i + 0] & 0x8000) >> 15;
                int yzom = (pgm_spritebufferram[i + 1] & 0x7800) >> 11;
                int ygrow = (pgm_spritebufferram[i + 1] & 0x8000) >> 15;
                int palt = (pgm_spritebufferram[i + 2] & 0x1f00) >> 8;
                int flip = (pgm_spritebufferram[i + 2] & 0x6000) >> 13;
                int boff = ((pgm_spritebufferram[i + 2] & 0x007f) << 16) | (pgm_spritebufferram[i + 3] & 0xffff);
                int wide = (pgm_spritebufferram[i + 4] & 0x7e00) >> 9;
                int high = pgm_spritebufferram[i + 4] & 0x01ff;
                int pri = (pgm_spritebufferram[i + 2] & 0x0080) >> 7;
                int xzoom, yzoom;
                if (xgrow != 0)
                {
                    xzom = 0x10 - xzom;
                ...
                xzoom = ((pgm_videoregs[0x1000 + xzom * 4] * 0x100 + pgm_videoregs[0x1000 + xzom * 4 + 1]) << 16) | (pgm_videoregs[0x1000 + xzom * 4 + 2] * 0x100 + pgm_videoregs[0x1000 + xzom * 4 + 3]);
                yzoom = ((pgm_videoregs[0x1000 + yzom * 4] * 0x100 + pgm_videoregs[0x1000 + yzom * 4 + 1]) << 16) | (pgm_videoregs[0x1000 + yzom * 4 + 2] * 0x100 + pgm_videoregs[0x1000 + yzom * 4 + 3]);
                boff *= 2;
                if (xpos > 0x3ff)
                    xpos -= 0x800;
                if (ypos > 0x1ff)
            ...
            pgm_sprite_source_offset = i;
        }
        private static void draw_sprites0()
        {
            int i;
            for (i = pgm_sprite_source_offset; i < 0x500; i += 5)
            {
                int xpos = pgm_spritebufferram[i + 0] & 0x07ff;
                int ypos = pgm_spritebufferram[i + 1] & 0x03ff;
                int xzom = (pgm_spritebufferram[i + 0] & 0x7800) >> 11;
        ...
        private static void draw_sprites()
        {
            int last_pgm_sptite_source_offset = 0;
            int i;
            for (i = 0; i < 0x500; i += 5)
            {
                if (pgm_spritebufferram[i + 4] == 0)
                {
                    last_pgm_sptite_source_offset = i;
                    break;
                }
            }
            for (i = last_pgm_sptite_source_offset; i >= 0; i -= 5)
            {
                int xpos = pgm_spritebufferram[i + 0] & 0x07ff;
                int ypos = pgm_spritebufferram[i + 1] & 0x03ff;
                int xzom = (pgm_spritebufferram[i + 0] & 0x7800) >> 11;
                ...
                if ((priority == 1) && (pri == 0))
                    break;*/
                if (xzoom == 0 && yzoom == 0)
                    draw_sprite_new_basic(wide, high, xpos, ypos, palt, boff, flip, pri);
                else
                    draw_sprite_new_zoomed(wide, high, xpos, ypos, palt, boff, flip, xzoom, xgrow, yzoom, ygrow, pri);
                //draw_sprite_new_zoomed(wide, high, xpos, ypos, palt, boff, flip, xzoom, xgrow, yzoom, ygrow);
            }
        }
        ...
        private static void pgm_tx_videoram_w(int offset, byte data)
        {
            ...
            col = (offset / 4) % 64;
            row = (offset / 4) / 64;
            Tmap.pgm_tx_tilemap.tilemap_mark_tile_dirty(row, col);
        }
        ...
        private static void pgm_bg_videoram_w(int offset, byte data)
        {
            ...
            col = (offset / 4) % 64;
            row = (offset / 4) / 64;
            Tmap.pgm_bg_tilemap.tilemap_mark_tile_dirty(row, col);
        }
        ...
        public static void video_update_pgm()
        {
            ...
            new_clip.min_y = 0x00;
            new_clip.max_y = 0xdf;
            Array.Copy(uuBFF, Video.bitmapbasePGM[Video.curbitmap], 0x40000);
            pgm_sprite_source_offset = 0;
            draw_sprites(1);
            Tmap.pgm_bg_tilemap.tilemap_set_scrolly(0, pgm_videoregs[0x2000] * 0x100 + pgm_videoregs[0x2000 + 1]);
            for (y = 0; y < 224; y++)
                Tmap.pgm_bg_tilemap.tilemap_set_scrollx((y + pgm_videoregs[0x2000] * 0x100 + pgm_videoregs[0x2000 + 1]) & 0x7ff, pgm_videoregs[0x3000] * 0x100 + pgm_videoregs[0x3000 + 1] + pgm_rowscrollram[y * 2] * 0x100 + pgm_rowscrollram[y * 2 + 1]);
            Tmap.pgm_bg_tilemap.tilemap_draw_primask(new_clip, 0);
            draw_sprites(0);
            Tmap.pgm_tx_tilemap.tilemap_set_scrolly(0, pgm_videoregs[0x5000] * 0x100 + pgm_videoregs[0x5000 + 1]);
            Tmap.pgm_tx_tilemap.tilemap_set_scrollx(0, pgm_videoregs[0x6000] * 0x100 + pgm_videoregs[0x6000 + 1]);
            Tmap.pgm_tx_tilemap.tilemap_draw_primask(new_clip, 0);
        }
